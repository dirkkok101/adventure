 ...
 * Orchestrates between mechanics services to handle turning light sources off.
 *
 * State Dependencies:
 ...
 *
 * Command Format:
 * - "switch [light source] off"
 * - Requires object and preposition (off)
 */
@Injectable({
  providedIn: 'root'
})
export class SwitchOffLightSourceCommandService extends BaseCommandService {
  /**
  ...
  override canHandle(command: GameCommand): boolean {
    ...
    return command.preposition === 'off' && !!command.object && !!command.target;
  }
  ...
  override handle(command: GameCommand): CommandResponse {
    ...
    const lightResult = this.lightMechanicsService.switchLightSourceOnOff(scene, object, false);
   ...
   * 4. Full verb and object: Suggest prepositions (off)
